 ...
 * choice below: Libra renders content it did not author. A transcript contains
 ...
 *  3. **Navigation lockdown.** The window can only ever show Libra's own UI.
 *     Every other URL is either opened in the user's real browser or dropped.
 *  4. **Network lockdown.** Chromium-side requests to anywhere but Libra's own
...
/**
 * `webPreferences` for every Libra window.
 *
...
export function windowSecurityPreferences(preloadPath: string, extraArguments: readonly string[]): WebPreferences {
  return {
    ...
    // `<webview>` is a second, weaker window with its own preferences. Libra has
    ...
    // them instead of asking main over IPC, which keeps `LibraBridge.version`
...
export function applySessionPolicy(session: Session, policy: SecurityPolicy): void {
  ...
  // on. Libra needs none of them, and a UI that renders untrusted content
...
export function hardenWebContents(contents: WebContents, policy: SecurityPolicy): void {
  ...
  // shows Libra's UI and nothing else, for its whole lifetime.
...
/**
 * Verify an IPC message came from Libra's own top-level frame.
 *
...
export function installNetworkAuthGuard(app: Electron.App): void {
  app.on('login', (event) => {
    ...
    // Libra never authenticates to a proxy or origin from the renderer, so a
