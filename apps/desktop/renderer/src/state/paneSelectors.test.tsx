 ...
 * identity holds, and they mount four real panes and assert React survives it.
...
import { quickModels, thinkingLevels, useApp } from '@/state/store';
...
describe('pane-scoped selectors under a split window', () => {
  ...
  it('holds every pane at once, not just the last two to render', () => {
    const panes = [0, 1, 2, 3].map((i) => paneWith(i % 2 === 0 ? 'sonnet' : 'opus'));
  ...
  it('renders a full four-pane grid without exceeding React’s update depth', () => {
    const panes = [0, 1, 2, 3].map((i) => createPane(paneWith(i % 2 === 0 ? 'sonnet' : 'opus')));

    useApp.setState({
      grid: [
        { id: 'rowA', panes: [panes[0] as never, panes[1] as never] },
        { id: 'rowB', panes: [panes[2] as never, panes[3] as never] },
      ],
    ...
    expect(() => {
      ...
      // refresh, a profile edit — puts all four columns through a render
      // together. That is the pass the one-slot cache could not survive.
      act(() => {
