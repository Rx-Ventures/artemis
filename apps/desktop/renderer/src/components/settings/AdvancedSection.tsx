 ...
 * ============================================================================
 *
 * The pane for what belongs to the *installation* rather than to the person
 * using it. The nav label says "This machine"; the component and the section
 * id keep their historical name — `advanced` is an address, and deep links
 * predate the rename.
 *
 * The old contract here was "arrangements Artemis hands you to run yourself",
 * and it stopped being the whole truth the day the update channel arrived:
 * that switch is a real setting the app applies. What the rows share now is
 * scope, not mechanism. Everything in this pane is a fact about this install
 * of Artemis on this computer — the symlinks on its disk, the folders it has
 * been worked in, the release channel its updater watches — and none of it
 * follows a profile or a project anywhere. Sign into the same accounts on
 * another machine and every row here starts over.
 *
 * ---------------------------------------------------------------------------
 ...
 *
 * In this pane rather than Appearance — where it first shipped — because it is
 * not a preference but a *record*: the app writes it as you work, and it
 * describes where this installation has been, which is exactly the kind of
 * fact the rest of this pane holds. It needs a pane at all because it is the
 * only place a folder can be taken back out. A folder opened once by mistake,
 * a client's repository that is no longer anyone's business, a throwaway
 * checkout: all of them sit in a menu the user opens twenty times a day, and
 * none of them can be got rid of from that menu.
 *
