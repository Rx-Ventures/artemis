 ...
 *
 * Shares the {@link ArtemisError} base with the rest of the engine so the main
 * process has exactly one `instanceof` check to make before turning a thrown
 * value into an `IpcFail`.
 */

import { ArtemisError } from '../profiles/errors.js';
 ...
 */
export class RunError extends ArtemisError {}
