/**
 * Claude ⇄ Libra translation, as pure functions.
 *
...
import { isPermissionMode } from '@libra/protocol';
...
export function mapSdkMessage(
  ...
  switch (message.type) {
    ...
    case 'auth_status':
      // Interactive-login progress. Every credential Libra uses — API key or
      // subscription token — is one the user obtained themselves and pasted in,
      // so Libra never drives an interactive auth flow and this should not fire
...
function mapSystemMessage(
  ...
  switch (message.subtype) {
    ...
    case 'hook_progress':
    case 'hook_response':
      // Hooks come from settings files. Libra runs with `settingSources: []` by
    ...
    case 'worker_shutting_down':
      // Infrastructure telemetry with no user-facing meaning in Libra.
      return [];
...
function mapUserMessage(message: UserMessage, state: ClaudeMapperState): readonly AgentEvent[] {
  ...
  if (typeof content === 'string') {
    // A plain string user turn. Libra's own prompts come back this way; the
...
function resolveRunEndReason(message: SDKResultMessage, state: ClaudeMapperState): RunEndReason {
  // Libra's own intent wins: if we asked the provider to stop, "interrupted" is
...
export interface BuildPermissionRequestParams {
  /** Libra's own id. Deliberately *not* the SDK's transport `requestId`. */
 ...
 * protocol. Libra mints its own id and keeps the pairing adapter-internal.
 */
 ...
 * `'cliArg'` destination has no protocol equivalent and Libra never produces
 * it.
 */
...
function fromSdkDestination(destination: PermissionUpdateDestination): PermissionScope | null {
  switch (destination) {
    ...
    case 'userSettings':
      return 'user';
    case 'cliArg':
      // No protocol equivalent, and offering it in the UI would suggest Libra
