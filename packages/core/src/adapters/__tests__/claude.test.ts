...
import type { AgentEvent, PermissionRequestEvent, RunEndEvent } from '@libra/protocol';
...
const MISSING_CWD = '/libra-tests/no-such-directory/at-all';
...
describe('buildClaudeOptions', () => {
  ...
  it('gives the provider the profile’s credentials, not the shell’s', () => {
    ...
    expect(env['CLAUDE_AGENT_SDK_CLIENT_APP']).toBe('libra');
  });
  ...
  it('sends the claude_code preset when the run named no system prompt', () => {
    // The default path for every Libra run: `RunInput.systemPrompt` is optional
